...
describe("Num: ", function () {
  describe("Num: ", function () {
    ...
    it("can check if a value is within a range set by two values", function () {
      assert.isTrue(
        Num.within(-3.001, 1, -3.001) && !Num.within(3.1, -100, 3.099),
      );
    });
  ...
  describe("Geom: ", function () {
    ...
    it("can interpolate between 2 points", function () {
      assert.isTrue(
        Geom.interpolate([10, 10], [20, 100], 0.3).equals([13, 37]),
      );
    });
    ...
    it("can rotate a group in 2D on YZ plane", function () {
      ...
      let s2 = Num.equals(ps[1].z, Math.sin(1.97568811) * 5.3851648 + 1);
      assert.isTrue(s1 && s2);
    });
    ...
    it("can shear a group in 2D on YZ plane", function () {
      ...
      assert.isTrue(
        Num.equals(ps[0].y, 218) &&
          Num.equals(ps[0].z, 482.324, 0.001) &&
          Num.equals(ps[1].z, 432.324, 0.001),
      );
    });
    ...
    it("can reflect a group in 2D on YZ plane", function () {
      ...
      assert.isTrue(
        Num.equals(ps[0].y, 274.14938) && Num.equals(ps[1].z, 497.471),
      );
    });
    ...
    it("can reflect a group in 2D on XZ plane", function () {
      ...
      assert.isTrue(
        Num.equals(ps[0].x, 274.14938) && Num.equals(ps[1].z, 497.471),
      );
    });
    ...
    it("can generate a random Pt from 2 Pt", function () {
      let a = Num.randomPt(
        new Pt(0.01, 0.01, 0.01),
        new Pt(0.015, 0.015, 0.015),
      );
      assert.isTrue(
        a[0] >= 0.01 &&
          a[0] < 0.015 &&
  ...
  describe("Range: ", function () {
    it("can calculate min and max of a range", function () {
      ...
      let r = new Range(d);
      assert.isTrue(
        r.max.equals(new Pt(10, 7, 9)) &&
          r.min.equals(new Pt(-100, -12, -40)) &&
          r.magnitude.equals(new Pt(110, 19, 49)),
      );
    });
    ...
    it("can normalize to unit", function () {
      ...
      assert.isTrue(
        Num.equals(target[0][0], 1) &&
          Num.equals(target[2][1], 15 / 19) &&
          Num.equals(target[4][2], 0),
      );
    });
    ...
    it("can normalize with exclude", function () {
      ...
      assert.isTrue(
        Num.equals(target[2][0], -100) &&
          Num.equals(target[5][1], 15 + ((99 - 15) * 7) / 19) &&
          Num.equals(target[3][2], -1),
      );
    });
    ...
    it("can append and recalculate", function () {
      ...
      let target = r.mapTo(0, 1);
      assert.isTrue(
        Num.equals(target[0][0], 110 / 1100) &&
          Num.equals(target[2][1], 23 / 320) &&
          Num.equals(target[4][2], 260 / 309) &&
          Num.equals(target[6][0], 1) &&
          Num.equals(target[7][2], 0),
      );
    });
    ...
    it("can create ticks from min and max range", function () {
      ...
      let t = r.ticks(10);
      assert.isTrue(
        t[0].equals(new Pt(-100, -12, -40)) &&
          t[3].equals(new Pt(-67, -6.30000019, -25.29999923)) &&
          t[10].equals(new Pt(10, 7, 9)),
      );
    });
  });
});
