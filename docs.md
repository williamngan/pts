...
Complete API reference for [Pts 1.0.1](https://ptsjs.org), generated from the same TypeDoc data used by the documentation website. Members are documented in full where they are declared and linked from subclasses that inherit them. For tutorials and runnable examples, see the [Pts guides and demos](https://ptsjs.org/guide.md).
...
```ts
new PoissonDisk().setup( space.innerBound, 12 ).sample( 40 )
```
...
```ts
static crop(shapes: PtLikeIterable | Iterable): Group[][]
```
...
- `shapes` (`PtLikeIterable | Iterable`) — an Array/Iterable of polygons in stacking order, back to front. Each is a Group or an Iterable<PtLike>, or a list of rings for a polygon with holes. A single ring is taken as one shape.
...
```ts
static divide(shapes: PtLikeIterable | Iterable): Group[][]
```
...
```ts
static exclude(shapes: PtLikeIterable | Iterable): Group[]
```
...
```ts
static intersect(shapes: PtLikeIterable | Iterable): Group[]
```
...
```ts
static minusBack(shapes: PtLikeIterable | Iterable): Group[]
```
...
```ts
static minusFront(shapes: PtLikeIterable | Iterable): Group[]
```
...
```ts
static unite(shapes: PtLikeIterable | Iterable): Group[]
```
...
<a id="svg-svgform-static-bezier"></a>
##### `bezier`

*static · overrides `CanvasForm.bezier`*
...
<a id="svg-svgform-static-bezier-element"></a>
##### `bezierElement`
...
<a id="svg-svgform-static-compound"></a>
##### `compound`

*static · overrides `CanvasForm.compound`*

**Overload 1**

```ts
static compound(ctx: DOMFormContext, rings: Iterable): SVGElement | undefined
```
...
<a id="svg-svgform-static-compound-element"></a>
##### `compoundElement`

*static*

```ts
static compoundElement(ctx: DOMFormContext, rings: Iterable): SVGElement | undefined
```
