...
First, we take every 2 points in `pts` to make 50 lines. Next, we make a line from space's center to pointer, and immediately turn it into an op of [`Line.intersectLine2D`](#op-line). Lastly we just apply the `hit` function to each pair and keep its intersection points.
...
[`Path`](#op-path) from "Op" module combines polygons with boolean operations. List the shapes from back to front; every function returns the rings of a polygon with holes, which `form.compound` draws as one path.
