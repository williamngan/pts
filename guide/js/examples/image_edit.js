...
(function () {
  ...
  let space = new CanvasSpace("#" + demoID).setup({
    retina: true,
    bgcolor: "#e2e6ef",
    resize: true,
  });
  ...
  img
    .load("/assets/img_demo.jpg")
    .then((res) => (imgform = new CanvasForm(res.ctx)));
    ...
    animate: (time, ftime) => {
      if (img.loaded) {
        ...
        form.image(
          [
            [0, 0],
            [space.size.x, img.canvas.height * scaling],
          ],
          img.canvas,
        );
        ...
        const r = Num.randomRange(
          0,
          space.pointer.$subtract(space.center).magnitude() * 2,
        );
        ...
        imgform
          .fill(`rgba(${c.join(",")})`)
          .stroke(Math.random() < 0.1 ? "#fff" : "#ffffff00");

        let size = new Pt(
          10 + Math.random() * 50,
          Math.min(space.size.x / 2, 30 + r),
        );
