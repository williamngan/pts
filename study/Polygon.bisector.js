...
space.add( {
  ...
  animate: (time, fps) => {
    ...
    form.line( [poly1[1], bisect] );

    form.stroke("#09f", 2);
    let bisect2 = Polygon.bisector( poly1, 2 );
    form.line( [poly1[2], bisect2] );
     
    form.stroke("#0c3", 2);
    let bisect3 = Polygon.bisector( poly1, 3, true, 1 ); // closePath and bisect not at middle but at the end
    form.line( [poly1[3], bisect3] );
